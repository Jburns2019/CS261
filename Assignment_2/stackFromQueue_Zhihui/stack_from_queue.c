/***********************************************************
* Author: Zhihui Liu & John Burns
* Email: liuzhih@oregonstate.edu
* Date Created: 7/14/19
...
// Single linked list with head and tail pointersw
struct Queue {
	struct Link* head;
	struct Link* tail;
};
...
{
	assert(queue != NULL);
	//queue->head = (struct Link*)malloc(sizeof(struct Link));//no need to create space for head & tail
...
*/
void listQueueAddBack(struct Queue* queue, TYPE value) // might have bugs
{
	assert(queue != NULL);
	...
	if (queue->head->next == NULL) {
		queue->head->next = newlnk;
		queue->tail = newlnk;
	}
	else {
		queue->tail->next = newlnk;
		queue->tail = newlnk;
	}
}
...
{
	assert(queue != NULL && queue->head->next != NULL);
...
*/
TYPE listQueueRemoveFront(struct Queue* queue) //might have bugs
{
	assert(queue != NULL && queue->head->next != NULL);
	struct Link *temp = queue->head->next;
	TYPE temp_val = temp->value;
...
struct Stack* listStackFromQueuesCreate()
{
	...
	stk->q1 = listQueueCreate();
	stk->q2 = listQueueCreate();
	return stk;
...
{
	assert(stack != NULL);
	struct Link *q2_head = stack->q2->head;
	struct Link *q2_tail = stack->q2->tail;
	stack->q2->head = stack->q1->head;
	stack->q2->tail = stack->q1->tail;
	stack->q1->head = q2_head;
	stack->q1->tail = q2_tail;
}
...
{
	assert(stack != NULL && listStackIsEmpty(stack) == 0);
